...
public class RadialPickerView extends View{
    ...
    private static final String[] HOURS_TEXT = new String[12], MINUTES_TEXT = new String[12];
    ...
    private float selectionCircleX, selectionCircleY, selectorRadius, selectorStroke;
    ...
    private void convertToText(){
        for (int i = 0; i < 12; i++) {
            HOURS_TEXT[i] = String.format("%d", HOURS_NUMBERS[i]);
    ...
    private void selectionCoordFromModel(){
        if(minutes){
    ...
    private void drawHours(Canvas canvas) {
        drawText(canvas,hourTextSize,HOURS_TEXT,hourX,hourY,1,hour12Paint);
    }
